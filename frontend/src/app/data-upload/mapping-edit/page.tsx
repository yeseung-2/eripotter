...
function MappingEditContent() {
  ...
  useEffect(() => {
    if (!normalId) {
      setErrorMessage('Normal ID가 제공되지 않았습니다.');
      return;
    }

    // normalId가 숫자인지 확인
    const numericId = parseInt(normalId);
    if (isNaN(numericId)) {
      setErrorMessage(`잘못된 Normal ID입니다: "${normalId}". 숫자 ID가 필요합니다.`);
      return;
    }
    ...
    (async () => {
      try {
        ...
        // 원본 데이터
        const nRes = await fetch(`/api/normal/substance/${numericId}`);
        if (!nRes.ok) {
          if (nRes.status === 404) {
            throw new Error(`Normal ID ${numericId}를 찾을 수 없습니다.`);
          }
          throw new Error(`원본 데이터 응답 오류 (${nRes.status})`);
        }
        ...
        const mRes = await fetch(
          `/api/normal/substance/mapping-results/${numericId}`
        );
        if (!mRes.ok) {
          if (mRes.status === 404) {
            throw new Error(`Normal ID ${numericId}의 매핑 결과를 찾을 수 없습니다.`);
          }
          throw new Error(`매핑 결과 응답 오류 (${mRes.status})`);
        }
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-red-700">
                <strong>데이터 로딩 오류:</strong> {errorMessage}
              </p>
              <p className="text-sm text-red-600 mt-1">
                페이지는 정상적으로 표시되지만, 실제 데이터는 로드되지 않았습니다.
              </p>
            </div>
          </div>
        </div>

        {/* Mock Data Display for Screenshot */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Summary bar */}
          <div className="bg-gradient-to-r from-gray-50 to-gray-100 border-b mb-6">
          ...
          <div className="bg-white rounded-lg shadow border mb-6">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <p className="mt-1 font-medium">리튬이온 배터리 팩</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">공급업체</p>
                <p className="mt-1 font-medium">LG화학</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">제조일자</p>
                <p className="mt-1 font-medium">2025-01-15</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-lg shadow-sm border p-5 ring-1 ring-green-200">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-medium text-gray-900">이산화탄소 (CO2)</h3>
                <span className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded-full">자동 매핑 완료</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  <p className="text-gray-500">원본 물질명</p>
                  <p className="font-medium">CO2</p>
                </div>
                <div>
                  <p className="text-gray-500">매핑된 물질명</p>
                  <p className="font-medium">Carbon dioxide</p>
                </div>
                <div>
            ...
            <div className="bg-white rounded-lg shadow-sm border p-5 ring-1 ring-yellow-200">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-medium text-gray-900">메탄 (CH4)</h3>
                <span className="px-2 py-1 text-xs bg-yellow-100 text-yellow-700 rounded-full">검토 필요</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  <p className="text-gray-500">원본 물질명</p>
                  <p className="font-medium">CH4</p>
                </div>
                <div>
                  <p className="text-gray-500">매핑된 물질명</p>
                  <p className="font-medium">Methane</p>
                </div>
                <div>
                  <p className="text-gray-500">신뢰도</p>
                  <p className="font-medium">87.1%</p>
                </div>
              </div>
            </div>
          </div>
