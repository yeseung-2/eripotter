...
class NormalService:
    ...
    def start_auto_mapping(self, normal_id: int, company_id: str = None, company_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 데이터베이스 세션 내에서 데이터 조회
            from eripotter_common.database import get_session
            from ..entity.normal_entity import NormalEntity
            
            with get_session() as db:
                # 세션 내에서 직접 조회하여 엔터티가 세션에 바인딩되도록 함
                normal_data = db.query(NormalEntity).filter_by(id=normal_id).first()
                if not normal_data:
                ...
                for i, gas_data in enumerate(greenhouse_gases):
                    ...
                    if ai_result.get("status") == "success":
                        ...
                        # 동일한 세션 내에서 매핑 결과 저장
                        success = self.normal_repository.save_ai_mapping_result_with_session(
                            db=db,
                            normal_id=normal_id,
                            gas_name=gas_name,
